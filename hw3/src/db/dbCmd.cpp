...
{
   // TODO...
   if((! cmdMgr->regCmd("DBAPpend", 4, new DBAppendCmd)) || (! cmdMgr->regCmd("DBAVerage", 4, new DBAveCmd)) || 
   (! cmdMgr->regCmd("DBCount", 3, new DBCountCmd)) || (! cmdMgr->regCmd("DBMAx", 4, new DBMaxCmd)) || 
   (! cmdMgr->regCmd("DBMIn", 4, new DBMinCmd)) || (! cmdMgr->regCmd("DBPrint", 3, new DBPrintCmd)) ||
   (! cmdMgr->regCmd("DBRead", 3, new DBReadCmd)) || (! cmdMgr->regCmd("DBSOrt", 4, new DBSortCmd)) || (! cmdMgr->regCmd("DBSUm", 4, new DBSumCmd)))
   {
...
DBAppendCmd::exec(const string& option)
{
   ...
   if (!CmdExec::lexOptions(option, key_value, 2))
	return CMD_EXEC_ERROR;
   if (!isValidVarName(key_value[0]))
   {
	cerr << "Error: Illegal option!! (" << key_value[0] <<")" << endl;
	return CMD_EXEC_ERROR;
   }
   int new_value;
   if (!myStr2Int(key_value[1], new_value))
   {
	cerr << "Error: Illegal option!! (" << key_value[1] <<")" << endl;
	return CMD_EXEC_ERROR;
   }
   if (!dbjson.add(DBJsonElem(key_value[0], new_value)))
   {
	cerr << "Error: Element with key \"" << key_value[0] <<"\" already exists!!" << endl;
	return CMD_EXEC_ERROR;	    
   }
