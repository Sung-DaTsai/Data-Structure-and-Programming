...
#define toSizeT(t)     (t % SIZE_T) ? SIZE_T * (t / SIZE_T + 1) : t // TODO
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  (t % SIZE_T) ? SIZE_T * (t / SIZE_T) : t  // TODO
...
class MemRecycleList
{
   ...
   void  pushFront(T* p) {
	  ...
	  _first = p;
	  size_t* y = (size_t*)p;
	  *y = 0; // let the last element in the list point to nullptr
      }
      else
      {
	  // get the address of the original first object in the list and turn it to be type size_t
	  void* v = (void*)_first;
	  size_t become_second = (size_t)v;

	  // get the address of the new first object in the list and turn it to be type size_t*
	  size_t* become_first = (size_t*)p;
          *become_first = become_second;
          _first = p;
      }
   }
   ...
   void reset() {
      // TODO
      MemRecycleList<T>* temps = this;
      size_t nums = numElm();
      for (size_t i=0; i<nums; ++i)
	  popFront();
      if (temps->_nextList != 0)
      {
          delete temps->_nextList;
	  temps->_nextList = 0;
      }
   ...
   size_t numElm() const {
      ...
      {
          ++ num;
	  temp_T = (T*)((void*)(*((size_t*)(temp_T))));  // the memory can't be returned, otherwise crash
      }
      return num;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      MemBlock<T>* temp_memblock = _activeBlock;
      MemBlock<T>* delete_memblock;
      while(temp_memblock->getNextBlock() != 0)
      {
	  delete_memblock = temp_memblock;
	  temp_memblock = temp_memblock->getNextBlock();
	  delete_memblock->reset();
	  delete delete_memblock;
      }
      temp_memblock->reset();

      if ((b!=0) && (b != _blockSize))
      {
	  delete temp_memblock;
          _blockSize = b;
          _activeBlock = new MemBlock<T>(0, b);
      }
      else
      {
	  _activeBlock = temp_memblock;
      }
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* temp_list = &_recycleList[m];
      while (true)
      {
	  if (temp_list->getArrSize() == n)
	      return temp_list;
	  if (temp_list->getNextList() == 0)
	      break;
	  temp_list = temp_list->getNextList();
      }
      temp_list->setNextList(new MemRecycleList<T>(n));
      return temp_list->getNextList();
   }
   ...
   T* getMem(size_t t) {
	  ...
	  throw bad_alloc(); // doesn't have any return, just jump to memCmd.cpp to be "catch"
      }
      ...
      {
	  if ((_activeBlock->getRemainSize()) >= S + static_cast<int>(SIZE_T))
	  {
		size_t rn = (_activeBlock->getRemainSize() - SIZE_T)/S;
		#ifdef MEM_DEBUG
		cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
		#endif
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t num_block = 1;
      MemBlock<T>* temp_blocks = _activeBlock;
      while (temp_blocks->getNextBlock() != 0)
      {
	  ++ num_block;
	  temp_blocks = temp_blocks->getNextBlock();
      }
      return num_block;
   }
